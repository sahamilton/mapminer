...
@foreach ($location->project->relatedNotes as $note)
	<td>{{$note->created_at->format('m-d-Y')}}</td>
	<td>@if($location->project->owned()  or auth()->user()->hasRole('Admin'))
...
@if($location->project->owned())
	<form method='post' action={{route('notes.store')}} name="noteForm">
		...
		<input type='hidden' name='related_id' value="{{$location->project->id}}" />
