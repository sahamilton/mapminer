...
<?php $rank = ($location->project->owner[0]->pivot->ranking ? $location->project->owner[0]->pivot->ranking: 3);?>
    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        
        <h4 class="modal-title">Transfer {!!$location->project->project_title!!} Project  </h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <div class="modal-body">
        <p>Please complete this form to transfer project</p>
        
        <form method="post" action="{{route('projects.transfer',$location->project->id)}}">
        {{csrf_field()}}
        <div class="form-group{{ $errors->has('comments') ? ' has-error' : '' }}">
         ...
         <div class="form-group{{ $errors->has('comments') ? ' has-error' : '' }}">
           ...
           <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button> <input type="submit" value="Transfer Project" class="btn btn-danger" />
            ...
            <input type="hidden" name="project_id" value="{{$location->project->id}}" />
