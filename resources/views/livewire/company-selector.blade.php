<div>
    <legend>Industry / Company {{implode(",",$vertical)}} {{count($vertical)}}</legend>
        ...
        <div class="input-group input-group-lg ">
            @foreach($verticals as $descendant)
                ...
                @if($descendant->type == 'group'  )
                    ...
                    @php $levelName = $descendant->filter; $n=4; @endphp
                    @if(! $loop->first)
                        </li></ul>
                    @endif
                ...
                @else
                    ...
                    @if((is_array(old('vertical')) && in_array($descendant->id,old('vertical'))) or (isset($activity) && in_array($descendant->id, $activity->vertical->pluck('id')->toArray())))
                        <input type="checkbox"  checked wire:model="vertical" name='vertical[]' value="{{{$descendant->id}}}"/>
                    @else
                        <input type="checkbox"  wire:model="vertical" name='vertical[]' value="{{{$descendant->id}}}"/>
                    @endif
