...
$rank = ($location->salesteam->first()->pivot->rating ? $location->salesteam->first()->pivot->rating: 3);@endphp
    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        
        <h4 class="modal-title">Close {!!$location->companyname!!} Lead  </h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <div class="modal-body">
        <p>Please complete this form to close lead</p>
        <?php $ratings = [1,2,3,4,5];?>
        <form method="post" action="{{route('mylead.close',$location->id)}}">
        {{csrf_field()}}
        <div class="form-group">
         ...
         <div class="form-group{{ $errors->has('comments') ? ' has-error' : '' }}">
           ...
           <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button> <input type="submit" value="Close Lead" class="btn btn-danger" />
            ...
            <input type="hidden" name="lead_id" value="{{$location->id}}" />
