...
$rank = ($location->lead->salesteam->first()->pivot->rating ? $location->lead->salesteam->first()->pivot->rating: 3);@endphp
    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        
        <h4 class="modal-title">Close {!!$location->lead->companyname!!} Lead  </h4>
        ...
        <div class="form-group">
         ...
         <div class="form-group{{ $errors->has('comments') ? ' has-error' : '' }}">
           ...
           <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button> <input type="submit" value="Close Lead" class="btn btn-danger" />
            ...
            <input type="hidden" name="lead_id" value="{{$location->lead->id}}" />
