...
$.fn.storeLocator = function(options) {
  ...
  $(function(){
		...
		if(settings.autoGeocode === true) {
			settings.dataLocation = settings.dataLocation +'/'+ orig_lat + ':' + orig_lng;
		}
        //Process the data
        $.ajax({
        type: "GET",
            ...
            if(settings.dataType === 'json' || settings.dataType === 'jsonp'){
              ...
              $(data).find('marker').each(function(){
                var locationData = {
                  ...
                  'industry_code':$(this).attr('industry_code'),
                  'lat': $(this).attr('lat'),
                  'lng': $(this).attr('lng'),
                  ...
                  'locationweb': $(this).attr('locationweb'),
				          'accountweb': $(this).attr('accountweb'),
          ...
          $(function(){ 
             ...
             var key, value, locationData = {};
              ...
              for(var y = 0; y <= storenum; y++) { 
                ...
                marker = createMarker(point, locationset[y]['name'], locationset[y]['address'], letter,locationset[y]['type'],locationset[y]['brand'],locationset[y]['color']  );
                marker.set("id", y);
                markers[y] = marker;
                if((settings.fullMapStart === true && firstRun === true) || settings.zoomLevel === 0){
                  bounds.extend(point);
