...
public function _import_csv($filename, $table,$fields)
	...
	public function isFiltered(array $searchtable = NULL, array $searchcolumn = NULL, $vertical = NULL)
	{
		...
		if(!isset($vertical)){
			...
			$allFilters = SearchFilter::whereIn('searchtable',$searchtable)->whereIn('searchcolumn',$searchcolumn)->where('type','=','group')->pluck('id');
		...
		//}elseif (isset($vertical)){
			...
			->orWhere(function($query) use($searchtable,$searchcolumn)
				{
					$query->whereIn('searchtable',$searchtable)->whereIn('searchcolumn',$searchcolumn)->where('canbenull','=',1);
			...
			->orWhere(function($query) use($searchtable,$searchcolumn)
				{
					...
					$query->whereIn('searchtable',$searchtable)->where('inactive','=',1)->whereIn('searchcolumn',$searchcolumn)->where('type','=','group');
					
				})
			->orWhere(function($query) use($searchtable,$searchcolumn)
			{
				
				$query->where('searchtable','=','locations')->where('inactive','=',1)->where('searchcolumn','=','segment')->where('canbenull','=',1);
		...
		if(count(array_intersect($searchFilters, $allFilters->toArray()))!= count($allFilters)){
